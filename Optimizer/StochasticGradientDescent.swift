...
public class StochasticGradientDescent {
	...
	private init(queue: MTLCommandQueue, pipeline: MTLComputePipelineState, η: Float, count: Int) {
		let groups: MTLSize = MTLSize(width: (count+15)/16, height: 1, depth: 1)
		let threads: MTLSize = MTLSize(width: 1, height: 1, depth: 1)
		optimizer = { (commandBuffer: MTLCommandBuffer, θ: MTLBuffer, Δθ: MTLBuffer) in
			assert(groups.width*16*MemoryLayout<Float>.size<=θ.length)
	...
	/*
	public static func factory(η: Float = 1e-3) -> (MTLCommandQueue) throws -> (Int) -> Optimizer {
		let bundle: Bundle = Bundle(for: self)
		let constantValues: MTLFunctionConstantValues = MTLFunctionConstantValues()
		constantValues.setConstantValue([η], type: .float, withName: "eta")
		return {
			let queue: MTLCommandQueue = $0
			...
			return {
				StochasticGradientDescent(: {queue.makeCommandBuffer()}, pipeline: pipeline, η: η, count: $0)
			}
		}
	}*/
	public static func factory(η: Float = 1e-3) -> OptimizerFactory {
		let bundle: Bundle = Bundle(for: self)
		let constantValues: MTLFunctionConstantValues = MTLFunctionConstantValues()
		constantValues.setConstantValue([η], type: .float, withName: "eta")
		return {
