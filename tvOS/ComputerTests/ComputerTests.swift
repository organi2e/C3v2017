...
class ComputerTests: XCTestCase {
	...
	func testGEMVGPU() {
		...
		do {
			...
			vDSP_vclr(yref, 1, vDSP_Length(N))
			*/
			vDSP_vgen([Float(-1)], [Float(1)], wref, 1, vDSP_Length(rows*cols))
	...
	func testGEMVCPU() {
		...
		do {
			...
			*/
			...
			measure {
				for _ in range.lowerBound..<range.upperBound {
					la_matrix_to_float_buffer(yref, la_count_t(1), la_matrix_product(la_elementwise_product(wmat, wmat), la_elementwise_product(xmat, xmat)))
				}
			}
			print("after", yref[rows-1])
		} catch {
