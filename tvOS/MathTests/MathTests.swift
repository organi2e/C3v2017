...
class MathTests: XCTestCase {
	...
	func testNonlinear() {
		...
		do {
			...
			do {
				...
				vDSP_vsadd(d.ref, 1, [Float(1.0)], d.ref, 1, vDSP_Length(count))
	...
	func testGEMV() {
		let cols: Int = uniform(128, 8192)
		let rows: Int = uniform(128, 8192)
		guard let device: MTLDevice = MTLCreateSystemDefaultDevice() else { XCTFail(); return }
		let queue: MTLCommandQueue = device.makeCommandQueue()
		...
		do {
			...
			do {
				...
				let rmse: Float = la_norm_as_float(e, norm) / Float(la_matrix_rows(e)*la_matrix_cols(e))
				XCTAssert(rmse<1e-2)
				print(rmse)
			}
			do {
			...
			do {
				let beg: Date = Date()
				for _ in 0..<64 {
					cblas_sgemv(CblasRowMajor, CblasNoTrans, Int32(rows), Int32(cols), 1.0, w.ref, Int32(cols), x.ref, 1, 0, y.ref, 1)
				}
				let end: Date = Date()
				print("elapsed: ", end.timeIntervalSince(beg))
			...
			do {
				let beg: Date = Date()
				for _ in 0..<64 {
					cblas_sgemv(CblasRowMajor, CblasTrans, Int32(cols), Int32(rows), 1.0, w.ref, Int32(rows), x.ref, 1, 0, y.ref, 1)
				}
				let end: Date = Date()
				print("elapsed: ", end.timeIntervalSince(beg))
